...
def register_user(request):
    ...
    try:
        ...
        if not all(key in data for key in ["first_name", "last_name", "username", "email", "password"]):
...
def login_user(request):
    ...
    try:
        ...
        user = RegisteredUser.objects.filter(email=data.get("email")).first()
        print(f"User found: {user}") 
        if user and check_password(data.get("password"), user.hashed_password):
